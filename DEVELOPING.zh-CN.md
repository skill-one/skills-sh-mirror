...
6. 同步 owner 头像 —— 对索引背后每个仓库的 owner,把 GitHub 头像下载到 `avatars/{owner}.png|jpg`(扩展名取自响应的 content-type,`size=96` 控制体积),并写入 `owners.jsonl`(每个 owner 一行,含上游 URL 和本地路径)。owner 信息搭在第 4 步的仓库响应上,下载走头像 CDN(无需 token、不受 API 限速),两者都不产生额外的 GitHub API 成本。行的 URL 未变化(GitHub 仅在头像变更时更新其 `?v=` 参数)且文件在盘时不发任何下载;下载失败沿用上一轮的行。
...
- owner 头像仅在新 owner、头像 URL 变化(GitHub 在头像变更时更新 URL 的 `?v=` 参数)或本地副本缺失时下载;否则直接复用现有文件,不发请求。下载失败沿用上一轮的行——首次失败则写入 `avatar` 为 `null` 的行,下次运行重试。`avatars/` 会被清理为 `owners.jsonl` 引用的那组文件。
...
`verify.mjs` 是数据集被信任或上传前的门禁:每行可解析、id 唯一、按 installs 降序(并列时按 id 升序)、字段格式正确、无两行映射到同一目录名、索引行与磁盘目录双向严格对应(每行都有目录、无孤儿目录)、`stats.json` 存在且可解析并与索引一致、`trending.json` / `curated.json` 是格式正确的 id 列表、`repos.jsonl` 格式正确、按 repo 排序且与索引的仓库集合精确一致、`owners.jsonl` + `avatars/` 格式正确且与索引的 owner 集合一致(每个被引用的头像文件都存在、无孤儿文件)、无 `.tmp` 残留。本地快速上手:
...
- **`fetch-skills.yml`**——发布器。发布完全自动化,没有本地发布路径。每日 18:00 UTC 运行,也可随时手动触发:
  ...
  每次运行先把上一份 `dist` 快照还原进 `data/`——其 `skills.jsonl` 里的上游 hash 用来固定 `fetchedAt`、沿用未变化的审计结果、保留抓取失败技能的上一次内容,并让 `changed`/`added`/`removed` 计数描述的是本次运行而非空工作区——然后全量抓取作为每日金丝雀 → `verify.mjs` → `publish.mjs` 强制推送 [`dist` 分支](README.zh-CN.md#如何获取数据),每天一个提交(同日重复运行会 amend 当天的提交),历史按 `publish.mjs` 的 `--window` 裁剪(默认 30 个提交,一天一个,约一个月;重新落根的提交保留原始的 author/committer 日期),并为窗口内的每个快照打 `dist-<日期>` 标签(标签名不含 `/`,以便在 raw URL 中解析;窗口外的标签会一并删除,被裁掉的提交因此保持不可达)。每份快照还带一个只有一行、内容即自身 tag 的 `latest` 指针(见 [README.zh-CN.md](README.zh-CN.md#如何获取数据));若指针、提交主题和实际标签三者不一致,运行会失败。工作流用长效 `VERCEL_TOKEN` 现场换取新鲜 OIDC token(所需 secrets:`VERCEL_TOKEN`、`VERCEL_ORG_ID`、`VERCEL_PROJECT_ID`——后两项在 `vercel link` 后从 `.vercel/project.json` 复制);抓 star 读取仓库 secret `GH_TOKEN`(个人访问 token,映射为环境变量 `GITHUB_TOKEN`——用 `gh secret set GH_TOKEN` 配置)。
