...
- **Conflicts** — `conflicts` (`kind: list|content`), `resolve_conflict` (write a resolution; commits for Git-index conflicts only; destructive). See `references/conflict-resolution.md`.
...
Git or local stale-save conflicts freeze writes (409 `doc-in-conflict`). A flush-time 409 `stale-external-write` retains the edit in Yjs/recovery, not disk: resolve and re-read; never blindly retry an append. For `exec` lifecycle `status: 'conflict'`, load `references/conflict-resolution.md`.
