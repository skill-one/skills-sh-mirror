#!/bin/bash
# lib/json.sh — JSON utilities for agent config manipulation (uses $OV_PY)
# Check if a JSON config file has openviking MCP enabled
check_json_mcp() {
  "$OV_PY" -c "import json; cfg=json.load(open('$1')); exit(0 if cfg.get('mcp',{}).get('openviking',{}).get('enabled') else 1)" 2>/dev/null
}
# Extract the openviking MCP URL from a JSON config file
get_json_mcp_url() {
  "$OV_PY" -c "import json; cfg=json.load(open('$1')); print(cfg.get('mcp',{}).get('openviking',{}).get('url',''))" 2>/dev/null
}
