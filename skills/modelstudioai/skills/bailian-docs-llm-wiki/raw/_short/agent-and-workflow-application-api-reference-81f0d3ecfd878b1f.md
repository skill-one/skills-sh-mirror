...
-   如需通过 Responses API 调用，请参阅 [Responses API](raw/application-api-reference/application-call/openai-responses-api.md)。
...
长期记忆体 ID，参阅[CreateMemory](raw/_short/api-bailian-2023-12-29-creatememory-b8e710cdd3512a87.md)创建。相关文档：[长期记忆](https://help.aliyun.com/zh/model-studio/call-single-agent-application#de63036b85aj0)。
...
`base64`编码格式可构建为 [Data URL](https://www.rfc-editor.org/rfc/rfc2397)：`data:[MIME_type];base64,{base64_image}`。详细说明和代码示例见本文档右侧[视觉理解](raw/_short/agent-and-workflow-application-api-reference-81f0d3ecfd878b1f.md)章节。
...
-   或通过[CreateIndex](raw/_short/api-bailian-2023-12-29-createindex-8bc613c75f8af371.md)接口（仅支持非结构化知识库）返回的`Data.Id`。
...
-   或通过[AddFile](raw/_short/api-bailian-2023-12-29-addfile-8c254b3500bc50bc.md)接口导入文档返回的 ID。
...
-   或通过[ListChunks](raw/_short/api-bailian-2023-12-29-listchunks-30a6c87b93a583c6.md)接口获取。
...
-   或通过[DescribeFile](raw/_short/api-bailian-2023-12-29-describefile-020886c28a208bf2.md)接口获取。
...
此参数用于在[深度思考模型](raw/model-user-guide/model-experience/text-generation-model/deep-thinking.md)中切换思考模式和非思考模式。仅适用于**智能体应用**。
...
dashscope application create -a your-app-id -p "你好"
```

> 将 `your-app-id` 替换为实际的应用 ID。`-a` 指定应用 ID，`-p` 指定 prompt 内容。
...
**应用配置：**应用内需使用[图像与视频理解](raw/model-user-guide/model-experience/vision-model/vision.md)模型。相关文档：[文件问答](https://help.aliyun.com/zh/model-studio/call-single-agent-application#30619780ddy93)。
...
调用时将`has_thoughts`参数设置为True，即可在`thoughts`中查看插件调用、知识检索的过程，或[深度思考模型](raw/model-user-guide/model-experience/text-generation-model/deep-thinking.md)的思考过程。
...
当在控制台**智能体应用**中选择了[深度思考模型](raw/model-user-guide/model-experience/text-generation-model/deep-thinking.md)，并成功发布应用后，若在 API 调用时将 `has_thoughts` 参数设为 `True`，则模型的思考过程将在此字段中返回。

**reasoningContent**`string`

模型的思考过程。

当在控制台**工作流应用**中选择了[深度思考模型](raw/model-user-guide/model-experience/text-generation-model/deep-thinking.md)，并成功发布应用后，若在 API 调用时将 `has_thoughts` 参数设为 `True`，则模型的思考过程将在此字段中返回。

**action\_type** `string`

大模型返回的执行步骤类型。如API表示执行API插件、agentRag表示执行知识检索、reasoning表示执行[深度思考模型](raw/model-user-guide/model-experience/text-generation-model/deep-thinking.md)的思考过程。
...
节点执行状态：`success`（成功）/ `fail`（失败）/ `skip`（跳过，未执行）/ `executing`（执行中）。
