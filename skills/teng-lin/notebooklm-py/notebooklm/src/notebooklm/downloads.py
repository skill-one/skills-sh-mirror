...
from ._types.artifacts import ArtifactType
...
def resolve_download_format(
    kind: ArtifactType, output_format: str | None = None
) -> tuple[str, DownloadFormatSpec]:
    """Resolve a supported representation and its canonical extension/MIME pair.

    Omission selects the existing per-kind default. Fixed-format kinds accept
    that representation explicitly, but never reinterpret an unsupported value.
    This describes implementation support, not upstream account entitlement.
    """
    for entry in DOWNLOAD_REGISTRY:
        if entry.kind != kind:
            continue
        default = entry.format_default or entry.default_output.extension.removeprefix(".")
        representation = default if output_format is None else output_format
        formats = dict(entry.formats) if entry.formats else {default: entry.default_output}
        if representation not in formats:
            raise ValidationError(
                f"Unsupported download format {representation!r} for {kind.value}"
            )
        return representation, formats[representation]
    raise ValidationError(f"Artifact kind {kind!r} has no supported download representation")
