...
description: 当用户或 Agent 需要通过同花顺金融数据服务获取、查询、同步、分析或导出 A 股行情、财报、估值、指数、板块、公募基金、期货、期权、特色数据或本地 DuckDB 数据，或需要选择、安装、配置、诊断 REST API、MCP、hithink-finance CLI、Python SDK/marketdb 时使用。
---
...
| 基金资料、基金公司、基金经理、净值、收益、财务、持仓、持有人、基金资讯、基金回测、基金指标、QDII 额度、ETF 行情 | 公募基金 | 先区分 `fund-otc/fund-etf/fund-lof/fund-reits` 与能力边界；行情当前仅支持 ETF |
...
1. 先区分文档查询与执行任务：仅查询接口说明、参数、字段或示例时，直接读取下表对应的本地 reference，再按业务域定位详情；不进入凭据、连接、工具探测或更新流程。执行取数或诊断接入时继续以下步骤，明确数据、资产类别、时间范围、新鲜度、复权口径、结果规模和输出形式；只在缺失信息会显著改变结果时做一次简短确认。
...
5. 只读取下表对应的一个一级 reference。REST/MCP 按业务域首页 → 原子接口或工具文档逐层读取，已知目标可直接进入详情页；REST 实际请求前读取 API 入口的通用约定一次。读取访问范围与可用状态；MCP 调用前以当前连接的工具名和 schema 核对。
6. 执行后报告数据源、时间范围、口径、行数、输出路径与线上验证边界。

## Skill 低频自更新引导

每个 Agent 会话第一次执行取数或诊断接入时，默认静默尝试一次统一 Skill 更新；同一会话后续调用不再检查。用户设置 `HITHINK_FINANCE_NO_SKILL_UPDATE=1` 时跳过。
...
统一凭据不要求安装 CLI。准备执行远端取数或诊断认证时按以下顺序检查，找到后直接复用，不再提示用户配置：
...
- **能查基金吗？** 支持公募基金资料、公司、经理、披露、财务、净值、收益、持有人结构、公开资讯元数据、无状态在线回测、通用指标、QDII 额度、ETF 快照和日线；不支持申赎交易或基金推荐。
