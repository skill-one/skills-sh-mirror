...
By default, local files are uploaded to DashScope temporary storage (`oss://` URLs, 48-hour TTL). For production use, you can configure your own 阿里云 OSS bucket to store uploaded files with no expiration and no rate limits.
...
| `QWEN_TMP_OSS_REGION` | Yes | OSS region | `cn-beijing` |
| `QWEN_TMP_OSS_AK_ID` | No* | AccessKey ID for OSS | `Lxxxxx...` |
| `QWEN_TMP_OSS_AK_SECRET` | No* | AccessKey Secret for OSS | `xxxxx` |
| `QWEN_TMP_OSS_PREFIX` | No | Object key prefix (default: `qianwen-uploads`) | `ai/prod` |
| `QWEN_TMP_OSS_ENDPOINT` | No | Custom endpoint (overrides region) | `my-bucket.oss-cn-beijing.aliyuncs.com` |
...
QWEN_TMP_OSS_REGION=cn-beijing
