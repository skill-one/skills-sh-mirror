...
An empty or partial result is not evidence for any cause in this table. "Missing from a view" and
"not instrumented" are different claims, and only the second prescribes a fix. Before telling anyone a
service is uninstrumented, confirm it directly: query that service's spans over a window where it was
active. A service that returns spans IS instrumented, whatever a view shows.

Reporting "not instrumented" off an absence produces a work order to fix something that is not broken
— worse than no answer, because a confident, well-argued diagnosis gets acted on. When neither claim
is established, say which one you checked and what you could not determine.
