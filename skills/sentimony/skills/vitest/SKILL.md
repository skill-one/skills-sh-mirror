...
Trusted inputs are the user's request as stated - the inspection or test run they asked for, the scope they named (project root, test files, name patterns), and the flags they passed explicitly, in particular `--script <name>`, the opt-in that authorizes running a package.json script through the package manager with its `pre`/`post` hooks.
