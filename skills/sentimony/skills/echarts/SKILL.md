...
This skill owns ECharts mechanics. When a symptom such as a blank chart or a broken resize
resists the known causes below, root-cause investigation belongs to `debugging`, and browser
runtime evidence to `web-debug`; this skill supplies the ECharts knowledge they reason with.

## Setup

Install with the project's own package manager (`npm`, `pnpm`, `yarn`, or `bun`):

```bash
<package manager> add echarts            # core library (always)
<package manager> add echarts-for-react  # React wrapper (optional)
<package manager> add vue-echarts        # Vue 3 wrapper (optional)
```
...
- **Shell commands**: yes, scoped to reading the project and installing the charting dependencies this skill names - `echarts` and the optional framework wrappers, through the project's own package manager, plus inspecting `node_modules/echarts/package.json` for the major version. Install on the user's request; do not add `echarts-gl` or other dependencies to a project that has none while auditing.
