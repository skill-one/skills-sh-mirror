...
WATERCOLOR MOMENTS — an OPTIONAL flourish, not a default (offered because the Watercolor illustration skill is also active). Reach for it only when the brief genuinely calls for a handmade, painterly moment — organic, storybook, nature, gift, or title-card beats where a crafted watercolor being painted carries the feeling. Do NOT add it to UI walkthroughs, product demos, data or diagram motion, or any piece where the user hasn't asked for that aesthetic; if in doubt, leave it out.
HOW (when it fits): design the painting FIRST with the `paint_watercolor` tool (preview, look, critique, final — the "Watercolor illustration" skill has the method and the paint API). When the still image is right, copy the watercolor starter (`copy_starter_component` with `kind: "watercolor_kit.js"`), load it with an ordinary `<script src="./watercolor-kit.js"></script>` tag BEFORE the engine script, and move the painting calls into a stable function (p) => { ... } defined once at module scope (drop the paper()/saveFile lines; keep the same seed; never an inline arrow; don't call saveFile inside it; coordinates match the width x height sheet you pass). Then pick ONE of three surfaces, all pure functions of T that export by construction:
• Stroke by stroke (the default): `<WatercolorPainting painting={paintFox} from={CUES.Paint} to={CUES.Paint + 6} width={900} height={1200} />` — the picture assembles from its actual brushstrokes in painting order: the background wash blooms in, the body washes layer on, ink lines draw tip to tail, splatter lands last. Each stroke is a real layer stacked over the paper, so this reads as a painting being painted, not a zoom over a finished image. Size and place the sheet with style (it keeps its aspect ratio).
• Strokes as scene objects: when individual strokes should move, fly in, stagger to a beat, or react to something, take the painting apart yourself — `const L = useWatercolorLayers(paintFox, {width: 900, height: 1200})` gives L.count strokes (L.kind(i), L.span(i)); render `<WatercolorSheet layers={L} style>...</WatercolorSheet>` containing one `<WatercolorStroke index={i} at={progress} style={{transform, opacity}} />` per stroke and drive each at/transform from T with animate(). Overlapping strokes darken where they cross, as in the still; keep 'reserve' (erase) strokes in their painted place and order.
• One flat image: `<WatercolorReveal painting={paintFox} from to width height />` pre-bakes the whole painting as a single picture that paints on — the lightest option, and the one to zoom or pan over as one image; or pass `frames={[src, ...]}` to supply pre-baked frames yourself.
