...
| Heroku Scheduler, Railway cron | `insta --agent cron create <name> '<expr>' --service <compute> --path </your/endpoint>` — the source runs a **command**, this calls an **HTTP endpoint**, so the work moves into a route (see the scheduler row above) | |
