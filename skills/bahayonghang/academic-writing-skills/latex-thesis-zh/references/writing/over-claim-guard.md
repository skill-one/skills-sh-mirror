...
上面的阶梯是上限，不是目标。反向的失败是措辞**低于**证据已支撑的那一级：主张前先写免责句、限制句写在它限定的结果前面、对自己的数字用"遗憾的是 / 仅仅 / 仍明显落后于"、结论以新的自我否定收尾。这一方向由 `claim-forward` 模块处理（`references/modules/claim-forward.md`，指南见 `references/writing/claim-forward-zh.md`）。

让两个方向保持一致的规则：

- 把措辞**抬到**证据支撑的那一级，绝不越级。用上面的"反向校准"清单判断强表达是否被认可；若不被认可，只调句子顺序，动词不动。
- 绝不为显得果断而删除 caveat、不利对比或非主线结果。claim-forward 只改顺序与措辞；每条限制的内容都保留。
- 每条限制只写一次，写在讨论证据的地方。删掉*重复*的 caveat 是校准；删掉*唯一*的一份是过度声明。
- 把差距写成测量值（"落后理想上界 4.1 个百分点"），不写成道歉（"遗憾的是仍明显落后"）。
