...
## 10. 学院中文题注末标点（仅 yanshan-ee-2025）

学院清单要求中文图题和表题不含标点，题后不加标点。编号与题注之间空一个汉字、以及英文大小写，仍由人工按学校和模板核对。

`check_tables.py --school yanshan-ee-2025` 只检查表浮动体中的中文题注。
`check_references.py --school yanshan-ee-2025` 只检查 `figure` 一类非表浮动体。同一个浮动体只由一个脚本报告。
本模式不在 `check_references.py` 增加引文位置、页码或文献著录规则。

脚本只看中文主参数的可见正文是否以中文标点结束。可选短题注和 `\bicaption` 的第二参数不参加这项检查。
英文句点不是中文标点命中。代码、数学和引用键中的标点不算。无法确认中文主参数时留人工。
没有末标点的中文题注不报。默认和 `--school generic` 不启用。候选为 `[Script]`、Info/P3、`Meaning-Check: NEEDS-LLM`，不输出整句替换。
