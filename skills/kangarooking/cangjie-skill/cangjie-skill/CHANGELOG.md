...
### 2026-09-13 补充优化（版本号不变）

- **工具书友好的三重验证**：V1 改为来源充分性，单处完整机制可保留；V2 检验适用范围内的新输入执行；V3 检查任务增益，不把作者独创或反常识当通用门槛。
- **内容去向可审计**：分开记录 verified / reference / needs_review / rejected；原书关键任务清单独立于旧通过项，重要内容不静默消失。
- **可操作内容提取**：补齐流程、公式、单位、字段、排障与例题；合成演练明确标记，不冒充作者经历。
- **输出评测修复**：缺失输出计入分母，失败/未完成返回非零；增加 JSON 精确值和数值容差断言，隔离各变体产物。
- **资源随包交付**：single/pack 可携带显式声明的 UTF-8 脚本与 CSV/JSON/Markdown 模板，拒绝越界、符号链接及二进制。
- **回归与发布包**：增加 26 项自动化回归、10 个合成分流样本和 3 个输出任务示例；更新 DeepSeek Harness 包，新增通用 ZIP 及 SHA256。真实书籍效果仍需原文和宿主实测，不把合成样本当效果提升证据。
- **发布口径**：仍为 2.5.0；保留首次发布标签，刷新同一 Release 的安装附件。包内 `BUILD_INFO.json` 标注补包日期与源码提交，GitHub 自动源码归档仍对应原始标签。
