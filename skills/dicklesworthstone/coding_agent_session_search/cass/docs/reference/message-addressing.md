...
Minimal and summary field presets can omit identity fields needed to disambiguate
sessions. Use an explicit field list for exact follow-ups. `remote` and `all` are
search filters, not exact follow-up source identifiers. Preserve the source path
verbatim; a conversation ID must match both that path and any supplied source.

## Exactness and provenance

The message-addressed lane opens the canonical archive read-only. A single SQL
statement observes both conversation identity and its messages. It rejects
...
meaning for readable local JSONL. Do not pass search's `line_number` here.
Legacy archive-backed handling of provider/non-file sources remains unchanged;
new automation should use `--message-index` explicitly.

The two coordinates conflict, and `expand` requires one. A blank, malformed,
or out-of-range raw line now produces an error instead of substituting another
JSON record and marking it `is_target`. A valid explicit raw-line read still
works even when its content is unrelated to a previous query.
...
target payload. This is a time budget, not a memory limit.
