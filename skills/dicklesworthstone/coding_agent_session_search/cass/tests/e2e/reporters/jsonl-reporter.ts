...
function captureEnvironment(): E2eEnvironment {
  return {
    ...
    cass_version:
      process.env.CARGO_PKG_VERSION ?? execOrNull("cass --version")?.split(" ")[1] ?? undefined,
...
class JsonlReporter implements Reporter {
  ...
  private getTestInfo(test: TestCase): E2eTestInfo {
    ...
    const file = test.location?.file ? path.relative(process.cwd(), test.location.file) : undefined;
