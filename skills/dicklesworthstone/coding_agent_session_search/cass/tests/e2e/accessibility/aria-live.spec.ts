...
test.describe("ARIA Live Region Announcements", () => {
  test("page has appropriate live regions defined", async ({ page, exportPath }) => {
    ...
    if (liveRegions.length === 0) {
      console.log(
        "[a11y-warning] No ARIA live regions found - dynamic updates may not be announced",
      );
    }
  });
  ...
  test("copy action announces success", async ({ page, exportPath, context, browserName }) => {
    ...
    console.log(
      `[a11y] Copy feedback: found=${feedback.found}, hasLiveRegion=${feedback.hasLiveRegion}`,
    );

    if (feedback.found && !feedback.hasLiveRegion) {
      console.log(
        "[a11y-warning] Copy feedback lacks live region - not announced to screen readers",
      );
    }
  });
  ...
  test("theme toggle announces change", async ({ page, exportPath }) => {
    ...
    console.log(
      `[a11y] Theme toggle state: before=${JSON.stringify(beforeState)}, after=${JSON.stringify(afterState)}`,
    );
  ...
  test("error messages use alert role", async ({ page, encryptedExportPath, password }) => {
    ...
    console.log(
      `[a11y] Error message: found=${errorInfo.found}, hasAlertRole=${errorInfo.hasAlertRole}`,
    );
    ...
    if (errorInfo.found) {
      ...
      if (!errorInfo.hasAlertRole) {
        console.log(
          '[a11y-warning] Error message lacks role="alert" - may not be announced urgently',
        );
      }
    }
  });
  ...
  test("loading states have appropriate ARIA", async ({ page, encryptedExportPath, password }) => {
    ...
    console.log(
      `[a11y] Loading states: aria-busy=${loadingResult.hadAriaBusy}, loading-indicator=${loadingResult.hadLoadingState}`,
    );
...
test.describe("Focus Management", () => {
  test("focus moves to content after decryption", async ({
    page,
    encryptedExportPath,
    password,
  }) => {
    ...
    if (focusInfo.isBody) {
      console.log(
        "[a11y-info] Focus remains on body after decryption - consider moving to main content",
      );
    }
  });
    ...
    if ((await skipLink.count()) > 0) {
      ...
      if (isSkipLinkFocused) {
        ...
        const focusInMain = await page.evaluate(() => {
          ...
          return (
            active?.closest("main") !== null || active?.id === "main" || active?.id === "content"
          );
        });
