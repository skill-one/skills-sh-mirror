...
test.describe("axe-core Automated Accessibility Audit", () => {
  ...
  test("encrypted export passes axe-core after decryption", async ({
    page,
    encryptedExportPath,
    password,
  }) => {
  ...
  test("dark theme passes axe-core", async ({ page, exportPath }) => {
    ...
    console.log(
      `[a11y] Dark theme: ${results.violations.length} total violations, ${criticalViolations.length} critical`,
    );
