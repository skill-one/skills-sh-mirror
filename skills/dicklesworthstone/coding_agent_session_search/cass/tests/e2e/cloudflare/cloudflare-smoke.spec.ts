...
import { expect, test } from "../setup/test-utils";
...
const REPORT_DIR =
  process.env.CLOUDFLARE_REPORT_DIR || path.join(process.cwd(), "test-results", "cloudflare");
        ...
        if (actualValue) {
          ...
          if (!actualValue.toLowerCase().includes(expectedValue.toLowerCase())) {
            report.warnings.push(
              `Header ${header}: expected "${expectedValue}", got "${actualValue}"`,
            );
          }
        } else {
          ...
          report.remediations.push(
            generateRemediation(`missing-${header.replace("x-", "").replace(/-/g, "")}`),
          );
        }
      }
