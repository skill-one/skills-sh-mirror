...
test.describe("Print Styles", () => {
  ...
  test("print uses light background", async ({ page, exportPath }) => {
    ...
    if (rgbMatch && rgbMatch.length >= 3) {
      ...
      expect(["white", "transparent", "inherit"].includes(bgColor) || bgColor.includes("255")).toBe(
        true,
      );
    }
  });
  ...
  test("code blocks preserve formatting in print", async ({ page, exportPath }) => {
    ...
    if (preCount > 0) {
      ...
      const whiteSpace = await firstPre.evaluate((el) => window.getComputedStyle(el).whiteSpace);
