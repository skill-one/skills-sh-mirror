...
test.describe("Basic HTML Rendering", () => {
  test("renders complete HTML document structure", async ({ page, exportPath }) => {
    ...
    await expect(
      page.locator('header[role="banner"], .conversation-header, [data-testid="header"]').first(),
    ).toBeVisible();
    await expect(
      page.locator('main, .conversation, [data-testid="conversation"]').first(),
    ).toBeVisible();
  });
    ...
    if (count > 0) {
      ...
      const hasHighlighting = await page.evaluate(() => {
        ...
        const hasBgColor =
          ...
          window.getComputedStyle(code.closest("pre") ?? code).backgroundColor !==
            "rgba(0, 0, 0, 0)";
  ...
  test("renders all message roles correctly", async ({ page, exportPath }) => {
    ...
    const agentMessages = page.locator(
      '.message-agent, .message-assistant, [data-role="assistant"]',
    );
