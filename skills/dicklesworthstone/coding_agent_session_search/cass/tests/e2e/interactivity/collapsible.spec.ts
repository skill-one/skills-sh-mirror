...
test.describe("Collapsible Sections", () => {
  ...
  test("collapse all/expand all functionality", async ({ page, toolCallsExportPath }) => {
    ...
    const expandAllBtn = page.locator('button:has-text("Expand all"), [data-action="expand-all"]');
...
test.describe("Copy to Clipboard", () => {
  ...
  test.beforeEach(async ({ browserName }) => {
    test.skip(
      ...
      "Clipboard API not fully supported in file:// URLs",
    );
  });
  ...
  test("copy button shows feedback", async ({ page, context, browserName, exportPath }) => {
    ...
    const btnHasCopiedClass = await copyBtn.evaluate((el) => el.classList.contains("copied"));
