import { expect, test, waitForPageReady } from "../setup/test-utils";
...
test.describe("Service Worker Cache Behavior", () => {
  ...
  test("service worker responds with cached content offline", async ({
    page,
    previewUrl,
    browserName,
  }) => {
  ...
  test("service worker cache survives page close and reopen", async ({
    page,
    previewUrl,
    browserName,
    context,
  ...
  test("cache updates when new version is available", async ({ page, previewUrl }) => {
    ...
    const hasUpdateFlow = await page.evaluate(() => {
      ...
      return {
        ...
        hasWaiting: !!(
          navigator.serviceWorker as ServiceWorkerContainer & { waiting?: ServiceWorker }
        ).waiting,
      };
    });
...
test.describe("OPFS Persistence", () => {
  ...
  test("OPFS handles large data correctly", async ({ page, previewUrl }) => {
    ...
    console.log(
      `[perf] OPFS 1MB write: ${result.writeTimeMs?.toFixed(2)}ms, read: ${result.readTimeMs?.toFixed(2)}ms`,
    );
...
test.describe("Storage Quota", () => {
  test("storage quota information is available", async ({ page, previewUrl }) => {
    ...
    if (quotaInfo.supported) {
      console.log(
        `[info] Storage quota: ${quotaInfo.quota}, usage: ${quotaInfo.usage} (${quotaInfo.usagePercent}%)`,
      );
