...
async function fixture(t, count = 2003, content = index => `Message ${index}`, globals = {}) {
    ...
    `);
    const insert = sqlite.prepare("INSERT INTO messages VALUES (?, 1, ?, 'user', ?, NULL, NULL, NULL)");
    ...
    sqlite.exec("INSERT INTO messages VALUES (999999, 2, 0, 'user', 'Other conversation', NULL, NULL, NULL); PRAGMA query_only=ON");
    const queries = [];
    const handle = {
        close() {},
        prepare(sql) {
...
test('ID windows and immutable message bodies use bounded LRU caches', async t => {
    ...
    assert.ok(queries.filter(query => query.sql.includes('LIMIT')).every(query => query.rows <= 50));
});
...
test('explicit full traversal preserves every message without retaining every body', async t => {
    ...
    assert.deepEqual(visited, Array.from({ length: 2003 }, (_, i) => [100 + 7 * i, i]));
    assert.ok(messages.getCacheStats().messages <= 64);
    assert.ok(messages.getCacheStats().idPages <= 4);
    assert.deepEqual(Array.from(messages.map(message => message.content)).slice(-3), ['Message 2000', 'Message 2001', 'Message 2002']);
});

test('retained text obeys its budget and oversized individual bodies are not cached', async t => {
    const { messages, queries } = await fixture(t, 30, index => index === 29 ? 'x'.repeat(2 * 1024 * 1024) : 'x'.repeat(100000));
...
async function viewerFixture(t, count = 2003) {
    ...
    const errors = [];
    let serial = 0;
    const later = (callback, delay = 0) => { const id = ++serial; timers.set(id, { callback, delay }); return id; };
    class Element {
        constructor() {
            this.dataset = {};
            this.style = {};
        ...
        set textContent(value) { this._text = value; this._html = String(value).replaceAll('&', '&amp;').replaceAll('<', '&lt;').replaceAll('>', '&gt;'); }
    ...
    const document = {
        ...
        getElementById(id) {
            if (!nodes.has(id) && ['back-btn', 'copy-btn', 'error-back-btn', 'messages-list'].includes(id)) {
    ...
    const db = await fixture(t, count, undefined, {
        document, window, setTimeout: later, clearTimeout: id => timers.delete(id),
    ...
    for (const [name, exports] of Object.entries({
        './attachments.js': { createAttachmentElement() {}, getMessageAttachments: () => [], initAttachments: async () => null, reset() {} },
        './share.js': { copyTextToClipboard: async text => { clipboard.push(text); return true; } },
        './virtual-list.js': { VariableHeightVirtualList },
    })) {
        modules[name] = new vm.SyntheticModule(Object.keys(exports), function () {
            for (const [key, value] of Object.entries(exports)) this.setExport(key, value);
        }, { context: db.context });
    }
    const viewerSource = await readFile(new URL('../src/pages_assets/conversation.js', import.meta.url), 'utf8');
    const viewer = new vm.SourceTextModule(viewerSource, { context: db.context });
    await viewer.link(name => { assert.ok(modules[name], `unexpected import: ${name}`); return modules[name]; });
    await viewer.evaluate();
    viewer.namespace.initConversationViewer(container, () => {});
    return {
        ...db, viewer: viewer.namespace, container, nodes, lists, clipboard, errors,
        lock() { windowListeners.get('cass:lock')(); },
        flush(delay = Infinity) {
            for (const [id, task] of [...timers]) {
                if (task.delay <= delay) { timers.delete(id); task.callback(); }
            }
        },
    };
}
...
test('viewer hydrates only visible messages and jumps directly to a distant deep link', async t => {
    ...
    assert.equal(f.viewer.getCurrentConversation().message_count, 2003, 'actual count replaces stale metadata');
    f.flush(100);
