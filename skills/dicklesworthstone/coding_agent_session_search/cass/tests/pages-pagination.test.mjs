...
const databaseSource = await readFile(
  new URL("../src/pages_assets/database.js", import.meta.url),
  "utf8",
);
...
async function fixture(t, count = 137, globals = {}) {
  ...
  const message = sqlite.prepare(
    "INSERT INTO messages VALUES (?, ?, 0, 'user', ?, NULL, NULL, NULL)",
  );
  ...
  const handle = {
    prepare(sql) {
      ...
      return {
        bind(values) {
          params = values;
          execution.params = Array.from(values);
        },
        step() {
...
for (const searchMode of ["prose", "code", "auto"]) {
  test(`${searchMode} search reaches every match in stable non-overlapping pages`, async (t) => {
    ...
    const all = [0, 50, 100].flatMap((offset) =>
      ids(api.searchConversations(query, { searchMode, limit: 50, offset }), "message_id"),
    );
    assert.deepEqual(
      all,
    ...
    assert.deepEqual(
      ids(api.searchConversations(query, { searchMode, offset: 50 }), "message_id"),
      all.slice(50, 100),
    );
...
test("recent browsing is deterministic even when all timestamps are tied", async (t) => {
  ...
  assert.deepEqual(
    all,
    Array.from({ length: 137 }, (_, i) => 137 - i),
  );
...
test("agent, workspace and optional time filters apply before page boundaries", async (t) => {
  ...
  assert.deepEqual(
    ids(api.getConversationsByTimeRange(1000, 1000, 50, 50)),
    Array.from({ length: 50 }, (_, i) => 87 - i),
  );
...
test("combined full-text filters retain every selected result across pages", async (t) => {
  ...
  const all = [0, 50].flatMap((offset) =>
    ids(api.searchConversations("needle", { ...options, offset }), "message_id"),
  );
  assert.deepEqual(
    all,
    Array.from({ length: 69 }, (_, i) => 2 * i + 1),
  );
...
function testDom() {
  ...
  class Element {
    ...
    set className(value) {
      this._classes = new Set(value.split(/\s+/));
    }
    get className() {
    ...
    set innerHTML(html) {
      ...
      for (const token of html.matchAll(/<!--[^]*?-->|<\/?([a-z][\w-]*)\b([^>]*?)>/gi)) {
        ...
        if (!["input", "br", "hr", "img", "meta", "link"].includes(element.tagName))
          stack.push(element);
      }
    }
    ...
    matches(selector) {
      ...
      return (
        ...
        (match[2] === undefined || this.dataset.resultIndex === match[2])
      );
    }
...
async function searchFixture(t, count = 137) {
  ...
  const router = new vm.SyntheticModule(
    ["parseRouteIdSegment"],
    function () {
      this.setExport("parseRouteIdSegment", (value) =>
        /^\d+$/.test(value) && Number.isSafeInteger(Number(value)) ? Number(value) : null,
      );
    },
    { context: db.context },
  );
  const virtual = new vm.SyntheticModule(
    ["VirtualList"],
    function () {
  ...
  return {
    ...
    resultIds: () =>
      element("results-list")
        ...
        .map((card) => Number(card.dataset.conversationId)),
    async run(action) {
...
test("UI reaches results beyond 1000 while retaining only one page", async (t) => {
  ...
  assert.deepEqual(
    all,
    Array.from({ length: 1053 }, (_, i) => i + 1),
  );
...
test("UI pages filtered browsing and search with either time bound omitted", async (t) => {
  ...
  for (const route of [
    ...
    const selected = Array.from({ length: 137 }, (_, i) => i + 1).filter(
      (id) => !route.agent || id % 2 === 1,
    );
