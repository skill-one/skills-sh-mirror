...
fn database(directory: &Path, payload: &Value, blob_value: bool) -> Result<PathBuf> {
    ...
    conn.execute(
        "INSERT INTO ItemTable VALUES ('unrelated.extension', 'not a transcript')",
    )?;
    conn.close()?;
    Ok(path)
}
...
fn workspace(home: &Path, name: &str) -> Result<PathBuf> {
    ...
    fs::create_dir_all(&directory)?;
    fs::write(directory.join("workspace.json"), r#"{"folder":"file:///work/copilot%20project"}"#)?;
    Ok(directory)
}
...
fn registry_discovers_and_reads_workspace_sqlite_transcripts_without_mutation() -> Result<()> {
    ...
    let path = database(&directory, &json!([session("legacy", "copilotlegacyneedle")]), false)?;
    ...
    assert!(sources.iter().any(|s| s.source_path == directory.join("workspace.json")));
    ...
    assert_eq!(conversation.workspace.as_deref(), Some(Path::new("/work/copilot project")));
    ...
    assert_eq!(bundle(&path)?, before, "provider database/WAL/SHM must not change");
    Ok(())
}
...
fn shared_vscode_database_does_not_misattribute_other_chat_providers() -> Result<()> {
    ...
    let path = database(&directory, &json!([
        other, unknown, session("copilot", "admitted content")
    ]), false)?;
    let conversations = connector().scan(&context(home.path(), &path))?;
    assert_eq!(conversations.len(), 1);
...
fn append_log_then_flat_json_take_precedence_without_losing_legacy_only_history() -> Result<()> {
    ...
    fs::write(sessions.join("shared.json"), session("shared", "obsolete flat copy").to_string())?;
    fs::write(sessions.join("flat-wins.json"), session("flat-wins", "flat winner").to_string())?;
    let append_log = sessions.join("shared.jsonl");
    fs::write(&append_log, format!("{}\n", json!({
        "kind": 0, "v": session("shared", "append log winner")
    })))?;
    let conversations = connector().scan(&context(home.path(), home.path()))?;
    assert_eq!(conversations.len(), 3);
    let shared = conversations.iter()
    ...
    assert!(conversations.iter().any(|c| {
        c.external_id.as_deref() == Some("legacy-only") && c.messages[0].content == "legacy survives"
    }));
    Ok(())
}
...
fn global_database_and_explicit_source_scope_remain_distinct() -> Result<()> {
    ...
    database(&directory, &json!([session("hidden", "outside scope")]), false)?;
    let global = home.path().join(".config/Code/User/globalStorage");
    let path = database(&global, &json!([session("empty-window", "global history")]), false)?;
    let conversations = connector().scan(&context(home.path(), &path))?;
    assert_eq!(conversations.len(), 1);
    assert_eq!(conversations[0].external_id.as_deref(), Some("empty-window"));
...
fn standard_cli_indexes_and_searches_copilot_sqlite_history() -> Result<()> {
    ...
    let path = database(&directory, &json!([session("cli", "copilotlegacyneedle")]), false)?;
    let before = bundle(&path)?;
    ...
    let output = assert_cmd::Command::from_std(command(home.path(), &data))
        .args(["search", "copilotlegacyneedle", "--mode", "lexical", "--json",
    ...
    assert!(!hits.is_empty(), "an empty success does not prove legacy ingestion: {value}");
    ...
    assert_eq!(bundle(&path)?, before, "CLI changed provider bytes or mtimes");
    Ok(())
}
