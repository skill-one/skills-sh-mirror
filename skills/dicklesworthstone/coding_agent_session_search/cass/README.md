...
|------------|-----------------|
| `frankensqlite` / `fsqlite-types` | crates.io `=0.3.16` (bumped from 0.3.14 on 2026-09-04; carries the GH#382 appended-WAL-tail fix — 8d012706a, the per-page WAL rescan that looped large-archive writable opens — plus the GH#405 FTS5 savepoint undo log, the GH#406 content-backed INSERT as an incremental segment append, lazy contentless FTS5 on ordinary opens, and the asupersync 0.4.3 migration, GH#333/GH#334 fixes including cass#393, allocator/freelist/concurrent-writer correctness work, FTS5 hardening, Windows sidecar-less read-only close repair, checkpoint/recovery and CHECK-enforcement hardening, and cass#434 autoindex-vanish corruption-writer fixes). The whole family resolves from one exact registry version; `build.rs` rejects any fsqlite-family registry patch, duplicate package resolution, wrong version, or non-crates.io lockfile source. The engine API is async; `src/franken_sync.rs` preserves cass's synchronous call shape via a current-thread asupersync `block_on` bridge. |
