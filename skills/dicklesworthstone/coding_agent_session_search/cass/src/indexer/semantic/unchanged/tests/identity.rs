...
fn refreshed_receipt_uses_the_existing_engine_cache_and_rejects_later_wal_edits() -> Result<()> {
    ...
    assert!(try_retain_completed(&inner, &storage, data, &manifest, &plan,
        &SemanticProgressSink::disabled())?.is_none(), "a cache hit must skip the new full proof");
    let second = indexer.run_backfill_from_storage(&storage, data, &mut manifest, plan.clone())?;
    assert!(second.unchanged);
    assert_eq!(fs::read(cache_path(data))?, receipt, "cache hits must not rewrite receipts");
    storage.raw().execute("UPDATE messages SET content = 'new content after cached proof' WHERE id = 1")?;
    assert_eq!(crate::indexer::lexical_storage_fingerprint_for_storage(&storage)?, plan.db_fingerprint);
    assert!(indexer.completed_backfill_fingerprint(&storage, data, &manifest,
        plan.tier, &plan.model_revision)?.is_none());
    assert!(prove_unchanged(&inner, &storage, data, &manifest, &plan,
        &SemanticProgressSink::disabled(), || Ok(()))?.is_none());
    assert_eq!(fs::read(cache_path(data))?, receipt, "a failed proof must not mint a new receipt");
    Ok(())
}
...
fn failed_receipt_write_does_not_fail_a_no_op_or_damage_serving_files() -> Result<()> {
    ...
    assert_eq!(fs::read(cache_path(data).join("keep"))?, b"not a writable cache");
    Ok(())
}
...
fn pinned_sql_snapshot_and_replaced_archive_path_cannot_borrow_current_file_stamps() -> Result<()> {
    ...
    storage.raw().query("SELECT content FROM messages WHERE id = 1")?;
    let before = snapshot(data)?;
...
#[test]
fn same_length_vector_edit_with_restored_mtime_invalidates_proof_and_does_not_refresh_cache() -> Result<()> {
    let temp = tempfile::tempdir()?;
    let data = temp.path();
...
fn foreign_revision_or_tombstoned_vector_image_is_not_a_completed_no_op() -> Result<()> {
    for tombstone in [false, true] {
        ...
        let records: Vec<_> = (0..reader.record_count()).map(|i| {
            Ok((reader.doc_id_at(i)?.to_owned(), reader.vector_at_f32(i)?))
        }).collect::<Result<_>>()?;
        drop(reader);
        let expected = engine::expected_vector_space_revision("fnv1a-384").unwrap();
        let revision = if tombstone { expected.to_owned() } else { "x".repeat(expected.len()) };
        let mut writer = VectorIndex::create_with_revision(&live, "fnv1a-384", &revision, 384, Quantization::F16)?;
        for (i, (id, vector)) in records.iter().enumerate() {
            if tombstone && i == 0 { writer.write_tombstone_record(id, vector)?; }
            else { writer.write_record(id, vector)?; }
        }
        writer.finish()?;
        assert_eq!(fs::metadata(&live)?.len(), manifest.quality_tier.as_ref().unwrap().size_bytes);
        let before = snapshot(data)?;
        let inner = engine::SemanticIndexer::new("hash", None)?;
        assert!(prove_unchanged(&inner, &storage, data, &manifest, &plan,
            &SemanticProgressSink::disabled(), || Ok(()))?.is_none());
