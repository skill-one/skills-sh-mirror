...
fn setup(
    ...
    manifest.hnsw = Some(HnswRecord {
        ...
        size_bytes: fs::metadata(&ann)?.len(),
        built_at_ms: manifest.updated_at_ms,
        ready: true,
    });
    manifest.save(data)?;
...
fn cache_path(data: &Path) -> PathBuf {
    data.join(VECTOR_INDEX_DIR)
        .join(".completed-backfill-quality-fnv1a-384.json")
}
...
#[test]
fn absent_corrupt_or_stale_cache_preserves_publication_ann_and_existing_and_fresh_search()
-> Result<()> {
    for mode in 0..3 {
        for capped in [false, true] {
            ...
            match mode {
                0 => {
                    fs::rename(cache_path(data), data.join("retained-cache.json"))?;
                }
                1 => {
                ...
                _ => {
                    storage
                        .raw()
                        .execute("CREATE TABLE unrelated_metadata (id INTEGER PRIMARY KEY)")?;
                }
            }
            let indexer = SemanticIndexer::new("hash", None)?;
            assert!(
                indexer
                    ...
                    .is_none(),
                "fixture must miss the skip cache"
            );
            ...
            assert_eq!(
                VectorIndex::open_read_only(&live)?.search_top_k(&query, 8, None)?,
                hits
            );
        }
    }
    ...
    for sql in [
        ...
        if !sql.starts_with("DELETE") {
            assert_eq!(
                ...
                plan.db_fingerprint,
                "negative must evade the coarse fingerprint"
            );
        }
        let before = snapshot(data)?;
        ...
        assert_eq!(
            snapshot(data)?,
            before,
            "failed proof itself never mutates the archive"
        );
        ...
        assert!(
            !manifest.hnsw.as_ref().unwrap().ready,
            "a real delta still revokes the old ANN"
        );
        let expected: HashSet<_> = engine::packet_embedding_inputs_from_storage(&storage)?
            .iter()
            .filter_map(engine::semantic_doc_id_for_input)
            .collect();
        let reader = VectorIndex::open_read_only(&outcome.index_path)?;
...
fn no_op_still_reclaims_interrupted_scratch_without_republishing() -> Result<()> {
    ...
    let staging = data
        ...
        .join(".staging-quality-minilm-384-deadbeef.fsvi");
    fs::copy(&live, &staging)?;
    let reuse = data
        .join(VECTOR_INDEX_DIR)
        .join(".backfill-reuse-Killed123");
    fs::create_dir(&reuse)?;
...
fn changed_archive_during_proof_refuses_before_publication_or_readiness_changes() -> Result<()> {
    ...
    let error = prove_unchanged(
        ...
        || {
            storage
                .raw()
                .execute("UPDATE messages SET content = 'changed during proof' WHERE id = 1")?;
            Ok(())
        },
    )
    .unwrap_err();
