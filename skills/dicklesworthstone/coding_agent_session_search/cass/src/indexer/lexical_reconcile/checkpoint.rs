...
fn valid_digest(value: Option<&str>) -> bool {
    value.is_some_and(|value| value.len() == 64
...
pub(super) fn resume(
    ...
    ensure!(current.version == VERSION && current.conversation_id > 0
        && valid_digest(current.projection_blake3.as_deref()),
        "invalid current lexical reconcile binding");
    let Some(previous) = previous else { return Ok(current); };
    ensure!(matches!(previous.version, 1 | VERSION),
        "unsupported lexical reconcile checkpoint version {}; checkpoint retained", previous.version);
    ensure!(previous.conversation_id == current.conversation_id
    ...
    ensure!(previous.message_count == current.message_count
    ...
    current.attempt = previous.attempt.checked_add(1)
...
mod tests {
    ...
    fn checkpoint(docs: &[CassDocument]) -> LexicalReconcileCheckpoint {
        LexicalReconcileCheckpoint {
            ...
            content_bytes: docs.iter().map(|doc| doc.content.len()).sum(), expected_docs: docs.len(),
            projection_blake3: Some(projection_fingerprint(docs)), started_at_ms: 100, attempt: 1,
        }
    }
    ...
    fn same_length_rewrites_and_every_projected_field_change_the_binding() {
        ...
        for field in 0..12 {
            ...
            assert!(resume(current, Some(expected.clone())).is_err(), "field {field}");
        }
    }
    ...
    fn fingerprint_framing_preserves_boundaries_options_and_order() {
        ...
        assert_ne!(projection_fingerprint(&[a.clone()]), projection_fingerprint(&[b.clone()]));
        assert_ne!(projection_fingerprint(&[a.clone(), b.clone()]), projection_fingerprint(&[b, a]));
        assert_ne!(projection_fingerprint(&[]), projection_fingerprint(&[document()]));
    }
    ...
    fn retries_keep_start_time_and_advance_attempt_without_rebinding() -> Result<()> {
        ...
        previous.attempt = 9;
        let mut current = previous.clone(); current.started_at_ms = 999;
    ...
    fn unsupported_misbound_and_malformed_checkpoints_cannot_resume() {
        ...
        for case in 0..11 {
            ...
            assert!(resume(current.clone(), Some(previous)).is_err(), "case {case}");
        }
    }
    ...
    fn legacy_checkpoint_is_explicitly_upgraded_before_full_replay() -> Result<()> {
        let current = checkpoint(&[document()]);
        let mut previous = current.clone(); previous.version = 1; previous.projection_blake3 = None;
    ...
    fn oversized_and_malformed_checkpoints_are_retained() -> Result<()> {
        ...
        for bytes in [vec![b' '; super::super::CHECKPOINT_MAX_BYTES as usize + 1], b"{unfinished".to_vec()] {
    ...
    fn invalid_conversation_identity_is_rejected_before_archive_or_lock_access() -> Result<()> {
        ...
        for id in [0, -1, i64::MIN] {
            let error = super::super::run_lexical_conversation_reconcile(&data, &data.join("archive.db"), id)
                .unwrap_err();
