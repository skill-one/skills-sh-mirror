...
async function unwrapDek(kek, slot, exportId) {
  ...
  try {
    ...
    // Import KEK
    const kekKey = await crypto.subtle.importKey("raw", kek, { name: "AES-GCM" }, false, [
      "decrypt",
    ]);
...
async function handleDecryptDatabase(dekBase64, cfg, requestId) {
  ...
  try {
    ...
    const dekKey = await crypto.subtle.importKey("raw", requestDek, { name: "AES-GCM" }, false, [
      "decrypt",
    ]);
    ...
    for (let i = 0; i < totalChunks; i++) {
      ...
      try {
        ...
        try {
          ensureCurrentDecryptAttempt(generation);
          const plaintext = await decompressDeflate(compressedChunk, payload.chunk_size);
          try {
            ensureCurrentDecryptAttempt(generation);
            const nextTotal = totalDecrypted + plaintext.byteLength;
            if (!Number.isSafeInteger(nextTotal) || nextTotal > payload.total_plaintext_size) {
        ...
        throw new Error(`Failed to decrypt chunk ${i}: ${error?.message || String(error)}`);
      } finally {
...
function validateSupportedPayloadFormat(cfg) {
  ...
  if (payload.chunk_size > MAX_ARCHIVE_CHUNK_SIZE) {
    throw new Error(
      `Invalid archive chunk_size: ${payload.chunk_size} exceeds maximum ${MAX_ARCHIVE_CHUNK_SIZE}`,
    );
  }
...
async function decompressDeflate(compressed, maximumOutputBytes) {
    ...
    try {
      while (true) {
        ...
        if (totalLength > maximumOutputBytes) {
          ...
          throw new Error(`Decompressed chunk exceeds ${maximumOutputBytes}-byte archive limit`);
        }
        chunks.push(value);
      }
      await writePromise;
  ...
  try {
    if (compressed.byteLength === 0) {
      ...
      const sliceSize = Math.min(FFLATE_INPUT_SLICE_SIZE, Math.max(1, maximumOutputBytes));
...
async function loadArgon2() {
  ...
  argon2LoadPromise = (async () => {
    self.loadArgon2WasmBinary = async () => {
      ...
      return readResponseBytesBounded(response, MAX_ARGON2_WASM_SIZE, "Argon2 WASM runtime");
    };
