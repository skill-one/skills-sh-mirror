...
function isUsableFutureExpiry(expiry, now) {
  return Number.isSafeInteger(expiry) && expiry > now && expiry - now <= MAX_TIMER_DELAY_MS;
}
...
export class SessionManager {
  constructor(options = {}) {
    this.duration = validateDuration(options.duration ?? SESSION_CONFIG.DEFAULT_DURATION_MS);
  ...
  async startSession(dek, rememberMe = false) {
    ...
    if (persistent) {
      ...
      try {
        ...
        if (!this.clearStorage()) {
          console.warn(
            "[Session] Failed session start left persisted data that could not be fully cleared",
          );
        }
    ...
    if (!isUsableFutureExpiry(expiry, Date.now())) {
      ...
      if (!this.clearStorage()) {
        console.warn(
          "[Session] Expired session start left persisted data that could not be fully cleared",
        );
      }
    ...
    try {
      ...
      if (!this.clearStorage()) {
        console.warn(
          "[Session] Unschedulable session start left persisted data that could not be fully cleared",
        );
      }
  ...
  endSession() {
    ...
    if (!storageCleared) {
      console.warn(
        "[Session] Session ended, but persisted session data could not be fully cleared",
      );
    }
