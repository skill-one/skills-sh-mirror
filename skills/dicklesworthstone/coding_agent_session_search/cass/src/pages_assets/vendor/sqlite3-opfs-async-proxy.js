(() => {
  ...
  const wPost = (type, ...args) =>
    postMessage({
      type,
      payload: args,
    });
  const installAsyncProxy = () => {
    const toss = (...args) => {
    ...
    const initS11n = () => {
      ...
      /**
			...
			*/
      state.s11n.deserialize = (clear = false) => {
        performance.now();
        const argc = viewU8[0];
        const rc = argc ? [] : null;
        if (argc) {
    ...
    /**
		...
		*/
    const getResolvedPath = (filename, splitIt) => {
    ...
    const getDirForFilename = async function f(absFilename, createDirs = false) {
      ...
      for (const dirName of path)
        if (dirName) dh = await dh.getDirectoryHandle(dirName, { create: !!createDirs });
      return [dh, filename];
    };
    /**
    ...
    const vfsAsyncImpls = {
      ...
      xDeleteNoWait: async (filename, syncDir = 0, recursive = false) => {
        let rc = 0;
        try {
          while (filename) {
      ...
      xOpen: async (fid, filename, flags, opfsFlags) => {
        const opName = "xOpen";
    ...
    if (isWebLocker) {
      ...
      vfsAsyncImpls.xLock = async (fid, lockType, isFromUnlock) => {
      ...
      vfsAsyncImpls.xUnlock = async (fid, lockType) => {
        const fh = __openFiles[fid];
        const existing = __activeWebLocks[fid];
        if (!existing) {
        ...
        if (lockType === state.sq3Codes.SQLITE_LOCK_NONE) {
          ...
          lockType === state.sq3Codes.SQLITE_LOCK_SHARED &&
      ...
      vfsAsyncImpls.xLock = async (fid, lockType) => {
        const fh = __openFiles[fid];
        let rc = 0;
        const oldLockType = fh.xLock;
        fh.xLock = lockType;
    ...
    const waitLoop = async function f() {
      if (!f.inited) {
        ...
        for (const k of Object.keys(state.opIds)) {
      ...
      while (!flagAsyncShutdown)
        try {
          ...
          await (f.opHandlers[opId]?.f ?? toss("No waitLoop handler for whichOp #", opId))(
            ...(state.s11n.deserialize(true) || []),
          );
        } catch (e) {
          error("in waitLoop():", e);
        }
