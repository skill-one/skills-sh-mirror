...
const LEGACY_SESSION_KEYS = ["cass_session_dek", "cass_session_expiry", "cass_unlocked"];
const LEGACY_SESSION_MANAGER_KEYS = ["cass_session", "cass_expiry", "cass_storage_pref"];
...
export function getArchiveScopeUrl() {
  try {
    ...
    const href = typeof window?.location?.href === "string" ? window.location.href : "unknown";
...
function restoreStorageEntries(storage, prefix, snapshot) {
  ...
  try {
    restored = storageEntriesEqual(snapshotStorageEntries(storage, prefix), snapshot) && restored;
  } catch (error) {
    ...
    case StorageMode.SESSION:
      {
        ...
        if (storage) {
          const storageCleared = removeStorageEntries(storage, (key) =>
...
export async function clearAllStorage(options = {}) {
  ...
  if (sessionStorageBackend) {
    if (allArchives) {
      sessionCleared = removeStorageEntries(
        sessionStorageBackend,
        (key) => key.startsWith(STORAGE_PREFIX) || isArchiveSessionKey(key),
      );
    } else {
      const archiveEntriesCleared = removeStorageEntries(sessionStorageBackend, (key) =>
        key.startsWith(archiveDataPrefix),
      );
      const sessionKeysCleared = removeStorageKeys(sessionStorageBackend, currentSessionKeys);
  ...
  if (localStorageBackend) {
    if (allArchives) {
      localCleared = removeStorageEntries(
        localStorageBackend,
        (key) =>
          (key.startsWith(STORAGE_PREFIX) &&
            (isArchiveDataEntryName(key) ||
              isArchivePreferenceKey(key) ||
              Object.values(LEGACY_PREF_KEYS).includes(key))) ||
          isArchiveSessionKey(key) ||
          isArchiveTofuKey(key),
      );
    } else {
      const archiveEntriesCleared = removeStorageEntries(localStorageBackend, (key) =>
        key.startsWith(archiveDataPrefix),
      );
...
export async function clearServiceWorkerCache(options = {}) {
  ...
  try {
    ...
    const cassNames = cacheNames.filter((name) =>
      allArchives ? name.startsWith("cass-archive-") : name.startsWith(cachePrefix),
    );

    const deleteResults = await Promise.allSettled(cassNames.map((name) => caches.delete(name)));
    for (let index = 0; index < deleteResults.length; index++) {
      const result = deleteResults[index];
      if (result.status === "rejected") {
        console.warn(
          "[Storage] Failed to delete Service Worker cache:",
          cassNames[index],
          result.reason,
        );
      }
    ...
    const remainingCassNames = remainingNames.filter((name) =>
      allArchives ? name.startsWith("cass-archive-") : name.startsWith(cachePrefix),
    );
    ...
    if (cleared) {
      ...
      console.warn(
        "[Storage] Some Service Worker caches could not be cleared:",
        remainingCassNames,
      );
    }
    return cleared;
...
export async function unregisterServiceWorker() {
  ...
  try {
    ...
    const unregisterResults = await Promise.allSettled(targets.map((reg) => reg.unregister()));
    for (let index = 0; index < unregisterResults.length; index++) {
      const result = unregisterResults[index];
      if (result.status === "rejected") {
        console.warn(
          "[Storage] Failed to unregister Service Worker:",
          targets[index].scope,
          result.reason,
        );
      }
    ...
    const remainingTargets = remainingRegistrations.filter((reg) => reg.scope === currentScope);
  ...
  if (isOPFSAvailable()) {
    try {
      ...
      for await (const name of root.keys()) {
        const isDatabaseResidue =
          currentArchiveDbFiles.has(name) || LEGACY_OPFS_DB_FILES.includes(name);
