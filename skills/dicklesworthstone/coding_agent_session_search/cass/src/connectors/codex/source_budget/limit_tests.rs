...
fn malformed_limits_never_become_unlimited_or_leak_the_setting() {
    for value in [
        "", " ", "0", "-1", "+1", "1.5", "NaN", "512MiB", "1_024",
        "1073741825", "18446744073709551616", "秘密", "\nprivate-value\n",
    ] {
...
fn opened_size_and_zero_output_validator_apply_the_same_exact_budget() -> Result<()> {
    ...
    let error = SourceSnapshot::capture_with_limit(&path, limit - 1)
        .err().expect("one byte over admission limit");
    assert_eq!(error.downcast_ref::<EnrichmentBudgetExceeded>().unwrap().observed_bytes, limit);
...
fn configured_preflight_refuses_before_parsing_or_completing_a_source() -> Result<()> {
    ...
    let ctx = ScanContext::with_roots(root.path().join("cass"),
        vec![franken_agent_detection::ScanRoot::local(path.clone())], None);
    let admission = ScanAdmission {
        exclusions: ScanExclusions::from_env(),
        limits: ScanLimits { jsonl_bytes: size - 1 },
    };
    ...
    let error = scan_with_admission(&franken_agent_detection::CodexConnector::new(), &ctx,
        &mut hooks, &mut |_| panic!("rejected source cannot emit"), &admission,
        |_| panic!("rejected source cannot enrich")).unwrap_err();
    let report = error.downcast_ref::<IncompleteScan>().unwrap();
...
fn legacy_refusal_records_its_lower_effective_limit() -> Result<()> {
    ...
    let mut state = ScanState { limits, incomplete: limits.incomplete(), ..ScanState::default() };
    ...
    assert_eq!(report["rejected_sources"][0]["limit_bytes"], MAX_AUGMENT_ROLLOUT_BYTES);
...
fn retained_handle_enrichment_reuses_tool_parser_and_rewinds_for_each_call() -> Result<()> {
    ...
    for _ in 0..2 {
        ...
        assert_eq!(output.messages[0].invocations[0].call_id.as_deref(), Some("call-1"));
        assert_eq!(output.messages[0].invocations[0].arguments,
    ...
    let tick = || {
        if !appended.swap(true, Ordering::SeqCst) {
            fs::OpenOptions::new().append(true).open(&path).unwrap()
                .write_all(tool_record().as_bytes()).unwrap();
        }
    };
    let error = snapshot.enrich(&mut conversation(&path), Some(&tick)).unwrap_err();
