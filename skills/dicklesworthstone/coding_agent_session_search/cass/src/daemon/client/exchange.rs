...
use crate::daemon::protocol::{ErrorCode, FramedMessage, PROTOCOL_VERSION, Request, Response, decode_message};
...
fn check(deadline: &Deadline) -> Result<(), DaemonError> {
    deadline.check().map_err(|error| io_error(error, "deadline"))
}
...
impl UdsDaemonClient {
    ...
    fn connection_until(&self, deadline: &Deadline) -> Result<MutexGuard<'_, Option<UnixStream>>, DaemonError> {
        // Do not alter another caller's stream/availability if admission expires.
        let mut connection = loop {
            check(deadline)?;
            if let Some(connection) = self.connection.try_lock() { break connection; }
            deadline.pause().map_err(|error| io_error(error, "connection admission"))?;
        };
        check(deadline)?;
        if connection.as_ref().is_some_and(|stream| stream.peer_addr().is_ok()) {
    ...
    fn open_until(&self, deadline: &Deadline) -> Result<UnixStream, DaemonError> {
        ...
        options.read(true).write(true).create_new(true).mode(0o600).custom_flags(libc::O_NOFOLLOW);
        ...
        check(deadline)?;
        let metadata = lock.metadata().map_err(|error| io_error(error, "spawn lock inspection"))?;
        if !metadata.is_file() || metadata.nlink() != 1 {
            return Err(DaemonError::Unavailable("refusing a non-regular or multiply-linked daemon spawn lock".into()));
        }
        loop {
            check(deadline)?;
            match lock.try_lock_exclusive() {
                ...
                Err(error) if error.kind() == io::ErrorKind::WouldBlock => {
                    deadline.pause().map_err(|error| io_error(error, "spawn lock admission"))?;
                }
        ...
        check(deadline)?;
        let binary = self.config.daemon_binary.clone().or_else(|| std::env::current_exe().ok())
            .ok_or_else(|| DaemonError::Unavailable("cannot determine daemon binary path".into()))?;
        remove_stale_daemon_socket(&self.config.socket_path)?;
        check(deadline)?;
        let mut command = Command::new(binary);
        command.arg("daemon").arg("--socket").arg(&self.config.socket_path);
        if let Some(data_dir) = &self.config.data_dir { command.arg("--data-dir").arg(data_dir); }
        let mut child = command.stdin(Stdio::null()).stdout(Stdio::null()).stderr(Stdio::null())
            .spawn().map_err(|error| io_error(error, "spawn"))?;
    ...
    fn wait_until(&self, child: &mut Child, deadline: &Deadline) -> Result<UnixStream, DaemonError> {
        loop {
            check(deadline)?;
            match child.try_wait().map_err(|error| io_error(error, "startup child poll"))? {
                Some(status) => return Err(DaemonError::Unavailable(format!("spawned daemon exited before becoming ready: {status}"))),
                None => {}
            }
            match transport::connect(&self.config.socket_path, deadline) {
                Ok(stream) => return Ok(stream),
            ...
            deadline.pause().map_err(|error| io_error(error, "startup wait"))?;
        }
    }
    ...
    pub(super) fn send_request(&self, request: Request) -> Result<Response, DaemonError> {
        ...
        let result = (|| -> Result<Response, DaemonError> {
            ...
            transport::write_all(stream, &encoded, &deadline).map_err(|error| io_error(error, "request write"))?;
            let mut prefix = [0; 4];
            transport::read_exact(stream, &mut prefix, &deadline).map_err(|error| io_error(error, "response prefix"))?;
            let length = u32::from_be_bytes(prefix) as usize;
            if length > MAX_FRAME_BYTES {
                return Err(DaemonError::Failed(format!("response too large: {length} bytes (max {MAX_FRAME_BYTES})")));
            }
            if length == 0 { return Err(DaemonError::Failed("empty daemon response frame".into())); }
            let mut bytes = Vec::new();
            bytes.try_reserve_exact(length).map_err(|_| DaemonError::Failed("cannot allocate daemon response frame".into()))?;
            bytes.resize(length, 0);
            transport::read_exact(stream, &mut bytes, &deadline).map_err(|error| io_error(error, "response payload"))?;
            let response = decode_message::<Response>(&bytes)
                .map_err(|_| DaemonError::Failed("failed to decode daemon response".into()))?;
            check(&deadline)?;
            if response.version != PROTOCOL_VERSION {
                return Err(DaemonError::Failed(format!("protocol version mismatch: expected {PROTOCOL_VERSION}, got {}", response.version)));
            }
...
fn validate_response_kind(request: &Request, response: &Response) -> Result<(), DaemonError> {
    let valid = matches!((request, response),
        (_, Response::Error(_))
        | (Request::Health, Response::Health(_))
        | (Request::ConnectionIdentity, Response::ConnectionIdentity(_))
        | (Request::HandshakeAttested { .. } | Request::HealthAttested { .. }, Response::Attestation(_))
        | (Request::EmbedAttested { .. } | Request::RerankAttested { .. }, Response::AttestedEmbedding(_))
        | (Request::Embed { .. }, Response::Embed(_))
        | (Request::Rerank { .. }, Response::Rerank(_))
        | (Request::Status, Response::Status(_))
        | (Request::SubmitEmbeddingJob { .. }, Response::JobSubmitted { .. })
        | (Request::EmbeddingJobStatus { .. }, Response::JobStatus(_))
        | (Request::CancelEmbeddingJob { .. }, Response::JobCancelled { .. })
...
fn reap_daemon(child: Child) -> Result<(), DaemonError> {
    ...
    match std::thread::Builder::new().name("cass-daemon-reaper".into()).spawn(move || {
        if let Some(mut child) = reaper.lock().take() { let _ = child.wait(); }
    }) {
        Ok(_) => Ok(()),
        Err(error) => {
