...
impl Deadline {
    ...
    pub(super) fn capped(&self, timeout: Duration) -> Self {
        Self {
            ...
            timeout: self.timeout.min(self.started.elapsed().saturating_add(timeout)),
        }
    }
    ...
    pub(super) fn remaining(&self) -> io::Result<Duration> {
        ...
        if remaining.is_zero() {
            Err(io::Error::new(io::ErrorKind::TimedOut, "daemon request deadline exceeded"))
        } else {
            Ok(remaining)
        }
    }
    ...
    impl Write for FrameWriter<'_> {
        fn write(&mut self, bytes: &[u8]) -> io::Result<usize> {
            ...
            let Some(total) = self.bytes.len().checked_add(bytes.len()) else {
                ...
                return Err(io::Error::new(io::ErrorKind::InvalidInput, "daemon request too large"));
            };
            ...
            if total > self.bytes.capacity() {
                let capacity = self.bytes.capacity().saturating_mul(2)
                    .max(total).min(MAX_FRAME_BYTES + 4);
                self.bytes.try_reserve_exact(capacity - self.bytes.len()).map_err(|_| {
                    self.failure = Some(io::ErrorKind::OutOfMemory);
                    io::Error::new(io::ErrorKind::OutOfMemory, "cannot allocate daemon request frame")
                })?;
            }
            self.bytes.extend_from_slice(bytes);
            Ok(bytes.len())
        }
        fn flush(&mut self) -> io::Result<()> { self.deadline.check() }
    }
    deadline.check()?;
    let mut writer = FrameWriter { bytes: vec![0; 4], deadline, failure: None };
    if message.serialize(&mut rmp_serde::Serializer::new(&mut writer)).is_err() {
...
pub(super) fn read_exact(stream: &mut UnixStream, mut bytes: &mut [u8], deadline: &Deadline) -> io::Result<()> {
    // Also applies to socket-pair tests or a connection installed by a caller.
    stream.set_nonblocking(true)?;
    while !bytes.is_empty() {
        deadline.check()?;
        match stream.read(bytes) {
            Ok(0) => return Err(io::Error::new(io::ErrorKind::UnexpectedEof, "daemon closed a partial frame")),
            Ok(count) => bytes = &mut bytes[count..],
            Err(error) if error.kind() == io::ErrorKind::Interrupted => {}
            Err(error) if matches!(error.kind(), io::ErrorKind::WouldBlock | io::ErrorKind::TimedOut) => deadline.pause()?,
...
pub(super) fn write_all(stream: &mut UnixStream, mut bytes: &[u8], deadline: &Deadline) -> io::Result<()> {
    stream.set_nonblocking(true)?;
    while !bytes.is_empty() {
        deadline.check()?;
        match stream.write(bytes) {
            Ok(0) => return Err(io::Error::new(io::ErrorKind::WriteZero, "daemon accepted no request bytes")),
...
pub(super) fn connect(path: &Path, deadline: &Deadline) -> io::Result<UnixStream> {
    ...
    if bytes.is_empty() || bytes.contains(&0) || bytes.len() >= address.sun_path.len() {
        return Err(io::Error::new(io::ErrorKind::InvalidInput, "invalid daemon socket pathname"));
    }
    ...
    { address.sun_len = u8::try_from(length).map_err(|_| io::Error::new(io::ErrorKind::InvalidInput, "daemon socket pathname is too long"))?; }
    let length = libc::socklen_t::try_from(length)
        .map_err(|_| io::Error::new(io::ErrorKind::InvalidInput, "daemon socket address is too long"))?;
    ...
    loop {
        ...
        let result = unsafe {
            libc::connect(stream.as_raw_fd(), std::ptr::from_ref(&address).cast(), length)
        };
        if result == 0 {
            deadline.check()?;
            return Ok(stream);
...
fn unconnected_stream() -> io::Result<UnixStream> {
    ...
    #[cfg(not(any(target_os = "linux", target_os = "android")))]
    {
        ...
        let result = unsafe { libc::fcntl(owned.as_raw_fd(), libc::F_SETFD, current | libc::FD_CLOEXEC) };
    ...
    #[cfg(any(target_os = "macos", target_os = "ios"))]
    {
        ...
        let result = unsafe {
            libc::setsockopt(stream.as_raw_fd(), libc::SOL_SOCKET, libc::SO_NOSIGPIPE,
                std::ptr::from_ref(&enabled).cast(), std::mem::size_of_val(&enabled) as libc::socklen_t)
        };
        if result < 0 { return Err(io::Error::last_os_error()); }
    }
    Ok(stream)
...
mod tests {
    ...
    fn bounded_encoder_preserves_wire_bytes_and_rejects_oversize_before_io() {
        ...
        for request in [Request::Health, Request::Embed {
            texts: vec!["résumé 日本語".into(), "ordered input".into()], model: "default".into(), dims: None,
        }] {
            let framed = FramedMessage::new("wire-test", request);
            assert_eq!(encode(&framed, &deadline).unwrap(), encode_message(&framed).unwrap());
        }
        let too_large = FramedMessage::new("large", Request::EmbeddingJobStatus { db_path: "x".repeat(MAX_FRAME_BYTES) });
        assert_eq!(encode(&too_large, &deadline).unwrap_err().kind(), io::ErrorKind::InvalidInput);
        assert_eq!(encode(&42, &Deadline::new(Duration::ZERO)).unwrap_err().kind(), io::ErrorKind::TimedOut);
    }
    ...
    fn trickled_bytes_do_not_renew_the_read_budget() -> io::Result<()> {
        ...
        let sender = std::thread::spawn(move || {
            for _ in 0..30 {
                if server.write_all(&[7]).is_err() { break; }
                std::thread::sleep(Duration::from_millis(20));
            }
        });
        let start = Instant::now();
        let error = read_exact(&mut client, &mut [0; 30], &Deadline::new(Duration::from_millis(100))).unwrap_err();
    ...
    fn stalled_reader_cannot_extend_write_deadline() -> io::Result<()> {
        ...
        let error = write_all(&mut client, &vec![0; MAX_FRAME_BYTES], &Deadline::new(Duration::from_millis(50))).unwrap_err();
        assert_eq!(error.kind(), io::ErrorKind::TimedOut);
        Ok(())
    }
    ...
    fn fragmented_reads_preserve_every_byte_and_eof_is_not_a_timeout() -> io::Result<()> {
        ...
        let sender = std::thread::spawn(move || -> io::Result<()> {
            for bytes in [&b"ab"[..], &b"c"[..], &b"def"[..]] { server.write_all(bytes)?; }
            Ok(())
        });
        ...
        assert_eq!(read_exact(&mut client, &mut [0], &deadline).unwrap_err().kind(), io::ErrorKind::UnexpectedEof);
        sender.join().unwrap()?;
        Ok(())
    }
    ...
    fn connect_is_nonblocking_and_accepts_non_utf8_pathnames() -> io::Result<()> {
        ...
        assert_eq!(client.read(&mut [0]).unwrap_err().kind(), io::ErrorKind::WouldBlock);
        assert_eq!(connect(&path, &Deadline::new(Duration::ZERO)).unwrap_err().kind(), io::ErrorKind::TimedOut);
        Ok(())
    }
    ...
    fn invalid_and_missing_socket_paths_fail_without_creating_files() -> io::Result<()> {
        ...
        assert_eq!(connect(Path::new(""), &deadline).unwrap_err().kind(), io::ErrorKind::InvalidInput);
        assert_eq!(connect(Path::new("a\0b"), &deadline).unwrap_err().kind(), io::ErrorKind::InvalidInput);
        assert_eq!(connect(&temp.path().join("missing"), &deadline).unwrap_err().kind(), io::ErrorKind::NotFound);
