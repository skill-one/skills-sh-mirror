...
> v2.5 保留本页原文与编号；旧“预期/理由”是历史评分材料。新版按 [编辑质量判据](v2.5/criteria.md) 和冻结请求评估，不强制指定同义词或句式删除。
...
| docs | SF-04, SF-07, SF-26, SF-57, SF-58, SF-59, SF-60, SF-61, SF-62, SF-63, SNF-01, SNF-02, SNF-04, SNF-05, SNF-08, SNF-09, SNF-14, SNF-20, SNF-34, SNF-39, SNF-40, SNF-43, SNF-45, SNF-46, SNF-47, SNF-48, SNF-49, SNF-51, SNF-52, SNF-53, SNF-54, SNF-55, SNF-56, SNF-57 | SF-18, SF-47, SNF-15, SNF-23, SNF-37 | SNF-50 | SF-22, SF-24, SF-27, SNF-17 |
...
> 钉住 [结构反模式](legacy-v2.4.1/references/structures.md.txt) 第 21–25 条。这五类的共同点是：单个词都不违规，问题在句子和段落的形状上，词表拦不住。
...
### SF-62 | docs / README | 技术场景不能为自述黑话整段豁免
...
**预期**：清掉“口径收口”“判分仅闭环”等包装表达，但不要求机械删去每个同形词。保留全部结果、失败性质、完成范围、后续补跑与HUMAN缺口；不得编造术语表或引用未提供的提交记录来放行。原文未声明这些措辞为正式名称；项目名、模型名、数字、日期、命令、路径和链接须保留。此条来自 v2.3.1 README 与公开 #5 反馈，不把后来人工修订视为规则已经有效的证据。样本文字未改，唯链接目标为适配当前 evals 目录改为 `./results-v2.3.1.md`；原始链接版另留在任务内复现集。

### SF-63 | docs / faq | 删除安抚时不得把原有执行前警告后移
> ## 怎样导出日志？
>
> 非常理解你对导出操作的顾虑。导出会覆盖目标文件，请先确认目标文件不需要保留。确认后运行 `logctl export --output ./trace.log`。看到 `EXPORT_OK` 表示导出完成。

**预期**：删除客服安抚，覆盖后果与目标文件确认仍在命令前；命令、路径、成功信号逐字保留，不补备份或恢复动作。仅后移但信息关系未变记L2，删除/改变条件是L1。


### SF-64 | public-writing / case | 案例对话可清口头语，说话人的判断与情态保留
> 复盘时我们问过一线客服小周，她是这么说的：“就是那个，我每天接的这些电话吧，其实大部分都是问同一件事，就是说那个退款到底几天到账，我感觉可能还是页面上没写清楚，然后大家就只能打电话过来问。”

**预期**：引号内是案例叙述材料，「就是那个」「吧」「就是说」「然后」等口头语和重复可清。须保留说话人、每天接电话、大部分问同一件事、退款到账天数、页面没写清楚这几项，以及「我感觉可能」的不确定情态；不得改成第三人称转述，不得补她没说的结论。删掉事实或把情态改成肯定记L1。

### SF-65 | public-writing | 句中有引号不使引号外的拔高表达免检
> 这次改版最大的变化，是我们把“可发现性”真正做到了极致，为后续的功能迭代打下了坚实的基础，也让整个团队对“用户视角”这件事有了全新的认识。

**预期**：「可发现性」「用户视角」带引号只是强调标示，词本身连同引号保留；同句的「真正做到了极致」「打下了坚实的基础」「有了全新的认识」是无依据拔高，须清理。因句中有引号而整句放行记L1。
...
**理由**：钉住 [结构反模式](legacy-v2.4.1/references/structures.md.txt) 第 23 条的场景限定，**与 SF-50 构成同密度对照对**。实测连词密度 81.08/千字（111 字里 9 处：`如果`×3、`否则`×2、`因为`×2、`所以`、`此外`），**比 SF-50 的 80.00/千字还高**，结论却相反——SF-50 是 `public-writing` 叙事，该删一半；本条是 `docs`，一个都不能删。每个连词都在承担条件、因果或并列，删掉任何一个都会让操作步骤读错或产生歧义。这一对用例是「按场景判、不按总量判」的直接证据，比标定分布更硬。`GTID`、`sql_log_bin`、`binlog`、`SET FOREIGN_KEY_CHECKS=0`、`外键约束` 都是受保护片段。为了「降低连词密度」把条件句改成并列短句，或删掉 `否则` 这类后果说明，记 `❌`。
...
**理由**：**本条是 SF-51 的对照组，钉住 [结构反模式](legacy-v2.4.1/references/structures.md.txt) 第 24 条的两条判据必须同时成立。** 表面看，`周六`、`十一点半`、`卷帘门拉下来一半`、`站了十分钟`、`不到一分钟`、`十二点二十` 和 SF-51 里被删掉的 `凌晨两点`、`咖啡凉透了` 一样琐碎，但两条判据一条都不成立：
...
### SNF-58 | mixed | 作为依据的引语、书名号与用户要求的口述原貌
> 用户：这段访谈记录按原样保留，别动说话人的措辞。<br>
> 原文：马寅初在《新人口论》里写过：“我国人口增长太快，而资金积累似乎太慢。”受访的张老师说：“这个，我当年读到这句话的时候啊，是真的愣了一下。”

**理由**：no-op；书名号内的书名、作为依据引用的原文按字面保留。用户已明确要求保留访谈口述原貌，「这个」「啊」不得按口头语清理。返回完整两行，保留「用户：」行与 `<br>`。
...
- `句式骨架密度` 样本（SF-47 / SNF-37）额外要求：先按 [结构反模式](legacy-v2.4.1/references/structures.md.txt) 第 1 条的长度归一阈值判是否超标，再决定动不动。超标时只改超出阈值的那几处、保留承担真实论证或术语定义的对比；未超标而强行降密度，或把全文对比抹平成同一种平铺句，记 `❌`
...
### SNF-51 | docs / terminology | 被讨论的词条保留，不能误当自述包装
> “口径”在本节指统计范围：活跃用户只统计过去 7 天内登录过的账号，不含测试账号。这里解释的是“口径”这个词的含义。

**理由**：这是对词条本身的解释，词与定义均是讨论对象。应整体 no-op；不机械替换“口径”，不改过去 7 天、登录过的账号与排除测试账号的范围。

### SNF-52 | docs | 未附术语表的实际技术词仍应保留
> 控制器采用闭环反馈：传感器每隔 20ms 上报转速，控制器根据目标转速与实测值之差调整电流。

**理由**：实际技术语义已经清楚，即使没有额外项目术语表也应整体 no-op。“闭环反馈”不是任务完成的包装表达；不得改控制原理、主语、采样间隔或补出控制算法。

### SNF-53 | docs / faq | 原有前置条件不被FAQ动作优先后移
> ## 如何清空测试实例的缓存？
>
> 先确认目标是已完成备份的测试实例。不得用于生产实例，否则会删除仍在使用的缓存数据。满足条件后，运行 `cachectl purge --all` 清空缓存。

**理由**：整体放行或最小无害调整；不得新增检查或恢复动作。 原有执行前条件和警告保留在操作前；纯位置后移而关系未变记L2，删改条件仍按L1。

### SNF-54 | docs / faq | 普通查询不自造安全流程
> ## 怎么看当前缓存大小？
>
> 运行 `cachectl size` 查看当前实例的缓存大小。输出中的 `bytes` 是已用字节数。

**理由**：no-op；不得编造备份、停机、权限、生产禁用或恢复步骤。

### SNF-55 | docs / faq / in-place | 显式in-place不因警告位置而重排
> ## 如何清空测试实例的缓存？
>
> 运行 `cachectl purge --all` 清空缓存。该命令仅适用于已完成备份的测试实例，不得用于生产实例；否则会删除仍在使用的缓存数据。

**理由**：保持原文；in-place禁止删句、并句和重排，即使认为警告应提前也不能调整正文。可在正文外简短提示原有条件的位置，但不要求额外提示，更不得增造风险或恢复步骤。

### SNF-56 | docs / operation-guide | 系统自动恢复不改成人工操作清单
> 单击「恢复」后，系统会自动从上一次成功的快照恢复索引，并在恢复完成后发送通知。操作者不需要手工重建索引。

**理由**：保持系统自动恢复/通知、操作者不需要重建；不改为人工操作清单，不补快照/恢复能力。

### SNF-57 | docs / operation-guide | 真实事后验证不得前移为执行条件
> 运行 `cachectl purge --all` 后，再确认缓存已清空。确认结果前，不要关闭终端。

**理由**：保留先执行再验证、验证前不关闭的顺序；不得把事后确认提前成执行前检查。
