...
> 2026-09-11：v2.5三文件重构已实作，本地最终候选仍有硬失败，未发布；范围与裁决见 [v2.5.0验证记录](results-v2.5.0.md)。以下旧运行各自保留原候选与历史口径。
...
## v2.4.1 旧候选：120 条改写与判分更正（2026-09-04 / 09-10）

- 候选 `61062b89c06a`，基线 `d2d0ce27da295581c3cf87a30ab65deb7d0ddfb8`。相对基线修改四份 references 的示例；120 条 benchmark 与 blind 哈希 `d91620417690e9d33d9722a341dc04aee2c6e403e6bcd9473dc1f8644d2dac19` 不变。
- Claude Opus 5（`claude-opus-5` / firstParty）与 Grok 4.6（`grok-4.6-build`，fingerprint `fp_08d0bc26c22b024e`）各完成 120 条改写，分 8 批。交叉 judge 覆盖 Claude 输出 120 条、Grok 输出 119 条；Grok 的 B-82 漏判，9 月 10 日由 Codex 静态补核通过，不记成交叉 judge 完成。
- 原调用总数 36，有效 33；认证和传输失败分别新建 `-r2` 目录续跑，漏判批保留 14 行归一化结果。旧目录、原始输出与 judge 不覆盖。最终纳入调用的实际模型、零工具、显式输入和持久会话输出一致性均核验通过。
- 9 月 10 日裁决：Grok SF-16 二元骨架倒装，风格 ❌；SF-37 丢失条件式交付动作，L1 ❌、风格 ❌。旧候选 L1 为 Claude 0/120、Grok 1/120，未达发布门槛。原“二者全通过”和“零保真失败”表述撤回。
- 更正后 SF 风格通过 Claude 53/63、Grok 49/63；SNF 误杀 2/57、1/57（后者含静态补核；原交叉 judge 为 1/56）。L3 三条两侧均通过，剔除后 L2 为 50/60、46/60。SNF-23 两位 judge 对同样删改判定不同，作为判分限制保留。
- 机械检查长文硬下限失败、目标下警告均为 0；Grok B61-75 的 6 条粗核报警为未复述 no-op 正文。机械检查没有发现 SF-37 的语义丢失，不等于保真通过。
- HUMAN 10 篇的代表性检查通过。段落统计只接入 residual，不修改硬判逻辑，也不参加 13 项规则输入的候选 ID；其事后统计与原始数据保留。
- 旧冻结物及批次：`tasks/current/v2.4.1-freeze-20260904/`、`v2.4.1-acceptance-20260904-*/`。更正记录：`tasks/current/v2.4.1-release-review-20260910/adjudicated-baseline.json`。公开摘录、分批数据与边界见 [results-v2.4.1.md](./results-v2.4.1.md)。

## v2.4.1 发布前定向修复（2026-09-10）

- 用户授权处理发布前剩余问题后，修正交付动作删除边界、效率效果类型和来源指导。另修 README 文案与能力边界、AGENTS 的安装路径说明。没有将旧全量数据作为新规则的成绩。
- 定向范围在首次调用前冻结：12 条既有回归 + 4 条任务内样本，覆盖四条旧锚点、条件式交付、效率表达、无源引用、引用保护和正常技术文本。任务内样本不计入公开 120 条，原文与预期记录保留。
- 第一轮 `8ec380ff6bb4`：两模型各完成 16 条改写，身份、零工具和输入一致性通过。静态复核确认 Claude 两条 L1（SF-16 决定关系弱化、B-201 能力情态丢失），Grok 两条 L1（SF-39 否定范围放宽、SF-07 技术与协作关系丢失）。因此未继续交叉 judge，也不重跑同候选追分。
- B-203 的任务内预期要求“明显提高”，但冻结规则允许删除没有指标的“显著”。该强度子项记预期冲突，不用于宣称强度保护通过；原文和预期不改，效率效果类型与不得补造数据仍按原合同复核。
- 第二轮 `a42417b70c58`：只在 `SKILL.md` 既有条目中修正二元对比默认删前半的指导，并明确决定性、能力情态及保真不确定时的保留要求。输入仍为同 16 条，未移除第一轮失败题。两侧改写完成；主 Agent 与独立审查 Agent 静态复核确认 Claude 的 SF-07 抽象命题丢失、SF-36 新增卡住前提，Grok 的 SF-16 决定关系弱化、SF-39 因果否定变成事实否定，均为 L1。
- 两轮合计 4 次改写调用，实际模型、零工具、显式输入及持久输出一致性均通过。Claude CLI 2.1.267，Grok CLI 1.0.24；两轮交叉 judge 均因已有 L1 而未执行，不宣称完成交叉判分或全量验收。
- 两轮修复已结束，当前候选未通过，不重跑同候选追分。维护者随后决定跳过 v2.4.1 独立发布，已有改动及后续保真修复转入 v2.5.0，历史结果不迁移为新版本成绩。原始记录和另存的静态裁决在 `tasks/current/v2.4.1-release-review-20260910/targeted-r1/` 与 `targeted-r2/`。
