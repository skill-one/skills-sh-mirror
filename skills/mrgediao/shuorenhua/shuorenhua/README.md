...
安装 full 时只复制 [runtime-files.json](runtime-files.json) 列出的三个文件，保持相对目录。不要把 `evals/` 和 `tasks/` 放进 skill 扫描目录。仓库开发文件不会自动更新已安装副本。
...
**长文为什么改得很少？** 这是已知缺口。在较长的公开文章里，2.5 对黑话和叠用比喻清理得偏少，常常只修了语病和标点。我们试过放宽清理范围，但会出现改动作者判断强度或作者自己术语的情况，还没达到发布标准，所以暂不发布，经过见 [#16](https://github.com/MrGeDiao/shuorenhua/issues/16)。2.4.x 清理得多一些，但也可能改变原意，如果使用，请逐处核对修订。
